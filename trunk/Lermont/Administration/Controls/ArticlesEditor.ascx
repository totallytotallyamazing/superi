...
<%--            <asp:ControlParameter ControlID="ihNodeID" DefaultValue="-1" Name="ScopeID" PropertyName="Value"
    ...
    <asp:PlaceHolder runat="server" ID="phPractice">
        Содержимое:
        <Controls:ResourceEditor runat="server" Type="RichText" RichHeight="400" ID="rePracticeText"></Controls:ResourceEditor>
        Изображения:
        <asp:DataList runat="server" ID="dlImages" OnItemDataBound="dlImages_ItemDataBound" OnItemCommand="dlImages_ItemCommand">
            <ItemTemplate>
                <asp:ImageButton runat="server" ID="ibPicture" CommandName="Delete"/>
            </ItemTemplate>
        </asp:DataList>
        <asp:PlaceHolder runat="server" ID="pImages">
            
        </asp:PlaceHolder>
        <asp:Button runat="server" ID="bUpdate" OnClick="bUpdate_Click" Text="Сохранить" />
    </asp:PlaceHolder>